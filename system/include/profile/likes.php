...
if($dot1 == $_SESSION['nLog']){
			echo"
			<div class='fiu_gh'><div class='uyi_poin'>Upload header image</div></div>
			";
}}
	echo"
		</div>
...
<?  
	$dot1 = $_GET['par'];
	$quertc50 = mysqli_query($link, "SELECT 1 FROM o_like WHERE `l_author`='{$dot1}'");
	$kc_mum = mysqli_num_rows($quertc50);
	if($kc_mum != 0){
	$query55 = mysqli_query($link, "SELECT l_master,l_author,l_object FROM o_like WHERE `l_author`='{$dot1}'");
	while ($row5 = mysqli_fetch_array($query55)){
	$ll_mast = $row5['l_master'];
	$ll_obj = $row5['l_object'];
		$q552 = mysqli_query($link, "SELECT * FROM o_track WHERE `o_url`='{$ll_obj}' and `type_dost`='1'");
			$vxb12 = mysqli_fetch_array($q552);
			$sodp_id = $vxb12['id'];
			$sodp_nm = $vxb12['o_name'];
			$sodp_nm = substr($vxb12['o_name'], 0, 191)." ...";
			$sodp_url = $vxb12['o_url'];
			$sodp_logo = $vxb12['o_lab'];
			$sodp_auth = $vxb12['o_author'];
	echo"		
			<div class='krug_g2 pprt1'>
				<div class='krug_4'>
						<img src='http://".$_SERVER['HTTP_HOST']."/content/octava/tracks/label/".$sodp_logo."' class='krug_im'>
					<div class='kijo'>
						<a class='dec_b' href='/".$ll_mast."'>".$ll_mast."</a>
					</div>
					<div class='kijo'>
						<a class='dec_b' href='/".$ll_mast."/".$sodp_url."'>".$sodp_nm."</a>
					</div>
