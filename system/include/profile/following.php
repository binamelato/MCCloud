...
<?  
	$dot1 = $_GET['par'];
	$quertc50 = mysqli_query($link, "SELECT 1 FROM o_followers WHERE `f_author`='{$dot1}'");
	$kc_mum = mysqli_num_rows($quertc50);
	if($kc_mum != 0){
	$query55 = mysqli_query($link, "SELECT DISTINCT f_master,f_author FROM o_followers WHERE `f_author`='{$dot1}'");
	while ($row5 = mysqli_fetch_array($query55)){
	$fif_author = $row5['f_master'];
		$query54 = mysqli_query($link, "SELECT login,avatar FROM profile WHERE `login`='{$fif_author}'");
		$row4 = mysqli_fetch_array($query54);
		$fif_aava = $row4['avatar'];
	
	echo"		
			<div class='krug_g2 pprt1'>
				<div class='krug_4'>
						<img src='http://".$_SERVER['HTTP_HOST']."/include/avatars/".$fif_aava."' class='krug_im'>
					<div class='kijo'>
						<a class='dec_b' href='/".$fif_author."'>".$fif_author."</a>
					</div>
		...
		echo"<div class='oioi_susu t0_zelist'></div>
				<div class='t1_menderl'>
					<p>".$dot1." isn’t following anyone yet.</p>
				</div>";
	}
?>			
	</div>
